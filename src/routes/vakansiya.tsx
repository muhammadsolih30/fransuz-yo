...
import type { Vacancy } from "../lib/vacancy-content";
...
function VacancyDetailCard({
  ...
  return (
    <article className={`vacancy-page__detail card p-4 sm:p-6 lg:p-10 ${className}`}>
      <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-start sm:justify-between gap-4 mb-5 sm:mb-6 pb-5 sm:pb-6 border-b border-[#15233B]/8">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-[#fcefec] to-[#f9ddd8] flex items-center justify-center shrink-0">
            <Icon className="w-6 h-6 sm:w-7 sm:h-7 text-[#e83848]" strokeWidth={1.8} />
          </div>
          <div className="min-w-0">
            <h2 className="font-['Syne'] font-extrabold text-lg sm:text-2xl leading-tight">{vacancy.title}</h2>
        ...
        <div className="bg-[#e83848] text-white rounded-2xl px-4 sm:px-5 py-2.5 sm:py-3 text-left sm:text-right w-full sm:w-auto shrink-0">
          ...
          <div className="font-['Syne'] font-bold text-sm">{vacancy.salary}</div>
        </div>
      </div>

      <p className="text-[#3E4B62] text-sm leading-relaxed mb-8">{vacancy.about}</p>
...
function VacancyPerksCard({
  ...
  return (
    <div className={`reveal card p-4 sm:p-6 panel-soft-accent ${className}`}>
      <h3 className="font-['Syne'] font-bold text-base mb-4">{title}</h3>
    ...
    <div className="vacancy-page bg-white text-[#15233B] pb-4 lg:pb-0">
      ...
      <section className="py-8 sm:py-12 lg:py-20 bg-[#FAF6EF]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Mobil: barcha vakansiyalar to'liq ochiq */}
          <div className="lg:hidden vacancy-page__mobile-list flex flex-col gap-4 sm:gap-5">
            {vacancies.map((vacancy) => (
              <VacancyDetailCard key={vacancy.id} vacancy={vacancy} ui={ui} />
            ))}
            <VacancyPerksCard title={ui.whyWorkTitle} perks={ui.perks} />
          </div>

          {/* Desktop: tab + bitta tanlangan vakansiya */}
          <div className="hidden lg:grid lg:grid-cols-12 gap-8 lg:items-stretch">
            <div className="lg:col-span-4">
              <div className="sticky-below-header self-start w-full">
                <div data-no-reveal className="vacancy-page__tabs flex flex-col gap-2 w-full">
                  {vacancies.map((v) => {
                    const Icon = v.icon;
                    return (
                      <button
                        ...
                        className={`text-left w-full flex items-center gap-3 px-5 py-4 rounded-2xl transition-all ${
                          active === v.id
            ...
            <div className="lg:col-span-8 min-w-0">
              <VacancyDetailCard key={current.id} vacancy={current} ui={ui} />
              <VacancyPerksCard title={ui.whyWorkTitle} perks={ui.perks} className="mt-6" />
            </div>
          </div>
        </div>
      </section>
